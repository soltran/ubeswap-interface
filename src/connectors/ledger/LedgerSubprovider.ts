...
const DEFAULT_BASE_DERIVATION_PATH = `44'/52752'/0'/0`
...
export class LedgerSubprovider extends BaseWalletSubprovider {
  ...
  /**
   * Instantiates a LedgerSubprovider. Defaults to derivationPath set to `44'/52752'/0'`.
   * TestRPC/Ganache defaults to `m/44'/52752'/0'/0`, so set this in the configs if desired.
  ...
  constructor(config: LedgerSubproviderConfigs) {
    ...
    this._baseDerivationPath = DEFAULT_BASE_DERIVATION_PATH
    ...
    this._addressSearchLimit =
      config.accountFetchingConfigs !== undefined && config.accountFetchingConfigs.addressSearchLimit !== undefined
   ...
   * @param basDerivationPath The desired derivation path (e.g `44'/52752'/0'`)
   */
  ...
  public async signPersonalMessageAsync(data: string, address: string): Promise<string> {
    ...
    try {
      const fullDerivationPath = derivedKeyInfo.derivationPath
      const result = await this._ledgerClientIfExists.signPersonalMessage(fullDerivationPath, stripHexPrefix(data))
